        ...
        document.addEventListener('alpine:init', () => {
            Alpine.data('data', () => ({
                ...
                init() {
                    ...
                    console.log('[INIT] Cards loaded:', this.svaip.cards.length);
                    console.log('[INIT] Cards data:', this.svaip.cards);
                    
                    // Debug: Expand the branches arrays to see actual data
                    this.svaip.cards.forEach((card, idx) => {
                        console.log(`[INIT] Card ${idx} branches:`, Array.from(card.branches || []));
                    });
                    
                    // Don't add a card on init when editing (cards already loaded)
                    if (this.svaip.cards.length === 0) {
                        this.addCard();
                    }
                    
                    this.$nextTick(() => {
                        console.log('[INIT] $nextTick - Setting up canvas');
                        this.canvas = this.$refs.canvas;
                        if (this.canvas) {
                            this.ctx = this.canvas.getContext('2d');
                            console.log('[INIT] Canvas and context initialized');
                            this.resizeCanvas();
                            window.addEventListener('resize', () => this.resizeCanvas());
                            
                            // Draw connections after delays to ensure DOM is ready
                            setTimeout(() => {
                                console.log('[INIT] First drawConnections attempt (100ms)');
                                this.drawConnections();
                            }, 100);
                            
                            // Add second attempt with longer delay
                            setTimeout(() => {
                                console.log('[INIT] Second drawConnections attempt (500ms)');
                                this.drawConnections();
                            }, 500);
                        } else {
                            console.error('[INIT] Canvas ref not found!');
                        }
                    });
                },
                ...
                finishConnection(index) {
                    if (this.connecting && this.connectFrom !== null && this.connectFrom !== index) {
                        console.log(`[CONNECT] Attempting to connect card ${this.connectFrom} side ${this.connectSide} to card ${index}`);
                        ...
                        console.log(`[CONNECT] Before: card ${this.connectFrom} branches:`, this.svaip.cards[this.connectFrom].branches);
                        this.svaip.cards[this.connectFrom].branches[this.connectSide] = index + 1;
                        console.log(`[CONNECT] After: card ${this.connectFrom} branches:`, this.svaip.cards[this.connectFrom].branches);
                        console.log(`[CONNECT] Set card ${this.connectFrom}.branches[${this.connectSide}] = ${index + 1}`);
                        this.$nextTick(() => this.drawConnections());
                    }
                    this.connecting = false;
                    this.connectFrom = null;
                ...
                drawConnections() {
                    console.log('[DRAW] drawConnections called');
                    console.log('[DRAW] Canvas:', this.canvas ? 'exists' : 'null');
                    console.log('[DRAW] Context:', this.ctx ? 'exists' : 'null');
                    console.log('[DRAW] Cards count:', this.svaip.cards.length);
                    
                    if (!this.ctx || !this.canvas) {
                        console.warn('[DRAW] Missing canvas or context, aborting');
                        return;
                    }
                    ...
                    // Find end card index (if exists)
                    const endCardIndex = this.svaip.cards.findIndex(c => c.type === 'end');
                    console.log('[DRAW] End card index:', endCardIndex);

                    let connectionsDrawn = 0;
                    this.svaip.cards.forEach((card, fromIndex) => {
                        console.log(`[DRAW] Card ${fromIndex}:`, {
                            type: card.type,
                        ...
                        if (card.type === 'end') {
                            console.log(`[DRAW] Card ${fromIndex} is end card, skipping`);
                            return;
                        }
                        
                        [0, 1].forEach(side => {
                            const targetIndex = card.branches ? card.branches[side] : null;
                            console.log(`[DRAW] Card ${fromIndex} side ${side}: target=${targetIndex}`);
                            
                            if (targetIndex !== null) {
                                const toIndex = targetIndex - 1;
                                if (toIndex >= 0 && toIndex < this.svaip.cards.length) {
                                    console.log(`[DRAW] Drawing arrow from ${fromIndex} to ${toIndex}`);
                                    this.drawArrow(fromIndex, toIndex, side);
                                    connectionsDrawn++;
                                } else {
                                    console.warn(`[DRAW] Target index ${toIndex} out of bounds`);
                                }
                            } else if (endCardIndex !== -1) {
                                // Branch is null AND end card exists → draw line to end card
                                console.log(`[DRAW] Branch is null, drawing to end card at index ${endCardIndex}`);
                                this.drawArrow(fromIndex, endCardIndex, side);
                                connectionsDrawn++;
                            }
                        });
                    });
                    
                    console.log(`[DRAW] Total connections drawn: ${connectionsDrawn}`);
                },
                    ...
                    try {
                        ...
                        this.svaip.cards.forEach((card, index) => {
                            if (card.type === 'end') {
                                ...
                                layout[questionIdx] = { x: parseInt(card.x) || 0, y: parseInt(card.y) || 0 };
                                questionIdx++;
                            }
                        });
                        
                        console.log('[SAVE] Layout:', layout);
                        console.log('[SAVE] Cards:', this.svaip.cards);
                        
                        // Format data for API - let api-service handle the conversion
                        ...
                        console.log('[SAVE] Final flowData:', flowData);
                        
                        // Call API
                        const response = await window.apiService.updateFlow(this.flowId, flowData);
                        
                        if (response.success) {
                            this.successMessage = 'Flow saved successfully!';
...
@section('content')
    <div class="px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-6">
            ...
            <h1 class="text-3xl font-bold text-gray-900">Edit Svaip</h1>
