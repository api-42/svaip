        ...
        document.addEventListener('alpine:init', () => {
            Alpine.data('data', () => ({
                ...
                drawArrow(fromIndex, toIndex, side) {
                    ...
                    // Draw arrowhead
                    const angle = Math.atan2(endY - (startY + controlY1) / 2, endX - startX);
                    const arrowSize = 12;
                ...
                snapAllCards() {
                    this.svaip.cards.forEach(card => {
                        card.x = this.snapToGrid(card.x);
                        card.y = this.snapToGrid(card.y);
                    });
                    this.drawConnections();
                },
                ...
                autoLayout() {
                    ...
                    levels.forEach((cardIndices, level) => {
                        const levelWidth = cardIndices.length * gridX;
                        const startOffsetX = startX + (1500 - levelWidth / 2);
                        
                        cardIndices.forEach((cardIndex, position) => {
                            this.svaip.cards[cardIndex].x = startOffsetX + (position * gridX);
                            this.svaip.cards[cardIndex].y = startY + (level * gridY);
                        });
                    });
                ...
                calculateLevels() {
                    ...
                    while (queue.length > 0) {
                        const [cardIndex, level] = queue.shift();
                        
                        if (visited.has(cardIndex)) continue;
                        visited.add(cardIndex);
                        
                        if (!levels[level]) levels[level] = [];
                        levels[level].push(cardIndex);
                        
                        const card = this.svaip.cards[cardIndex];
                        if (card.type !== 'end' && card.branches) {
                            [0, 1].forEach(side => {
                                const targetIndex = card.branches[side];
                                if (targetIndex !== null) {
                                    const targetCardIndex = targetIndex - 1;
                                    if (!visited.has(targetCardIndex) && targetCardIndex >= 0) {
                                        queue.push([targetCardIndex, level + 1]);
                                    }
                                }
                            });
                        }
...
@section('content')
    <div class="px-4 sm:px-6 lg:px-8">
        <form x-data="data()" action="{{ route('flow.store') }}" method="POST">
            ...
            <!-- Hidden fields for ALL cards -->
            <template x-for="(card, index) in svaip.cards" :key="index">
                <div>
                    <input type="hidden" :name="'cards[' + index + '][question]'" :value="card.question">
                    <input type="hidden" :name="'cards[' + index + '][description]'" :value="card.description || ''">
                    <input type="hidden" :name="'cards[' + index + '][options][0]'" :value="card.options[0]">
                    <input type="hidden" :name="'cards[' + index + '][options][1]'" :value="card.options[1]">
                    <input type="hidden" :name="'cards[' + index + '][x]'" :value="card.x">
                    <input type="hidden" :name="'cards[' + index + '][y]'" :value="card.y">
                    <template x-if="card.branches">
                        <div>
                            <input type="hidden" :name="'cards[' + index + '][branches][0]'" :value="card.branches[0] || ''">
                            <input type="hidden" :name="'cards[' + index + '][branches][1]'" :value="card.branches[1] || ''">
                        </div>
                    </template>
                    <template x-if="card.scoring">
                        <div>
                            <input type="hidden" :name="'cards[' + index + '][scoring][0]'" :value="card.scoring[0] !== undefined ? card.scoring[0] : ''">
                            <input type="hidden" :name="'cards[' + index + '][scoring][1]'" :value="card.scoring[1] !== undefined ? card.scoring[1] : ''">
                        </div>
                    </template>
                    <template x-if="card.skipable">
                        <input type="hidden" :name="'cards[' + index + '][skipable]'" value="1">
                    </template>
                </div>
            </template>
